    ...
    stages {
        ...
        stage('ldop-nexus-validate') {
            ...
            post {
                ...
                always { script { SUBJECT = "Build #${env.BUILD_NUMBER} of ${env.JOB_NAME} at 'ldop-nexus-validate'" } }
            }
        }
        stage('hadolint-lint') {
            agent {
        ...
        stage('ldop-nexus-build') {
            ...
            post {
                ...
                failure { script { STATUS = "FAILURE" } }
                changed { script { CHANGED = "YES" } }
                always { script { SUBJECT = "Build #${env.BUILD_NUMBER} of ${env.JOB_NAME} at 'ldop-nexus-build'" } }
            }
        }
        ...
        stage('ldop-image-deploy') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-nexus:${env.BRANCH_NAME} liatrio/ldop-nexus:${TAG}"
                sh "docker push liatrio/ldop-nexus:${TAG}"
            }
            post {
                success { script { STATUS = "SUCCESS" } }
                failure { script { STATUS = "FAILURE" } }
