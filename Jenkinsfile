    ...
    stages {
        ...
        stage('Build container') {
            ...
            steps {
                ...
                sh "docker build -t liatrio/ldop-nexus:${env.BRANCH_NAME} ."
                sh "docker push liatrio/ldop-nexus:${env.BRANCH_NAME}"
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-nexus:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-nexus",containerVersion)
                        sh "docker build -t liatrio/ldop-nexus:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Integration-test') {
            ...
            steps {
                ...
                withCredentials([[
                    ...
                    testSuite("ldop-nexus", "${env.BRANCH_NAME}", DIR)
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-nexus:${env.BRANCH_NAME} liatrio/ldop-nexus:latest"
                sh "docker push liatrio/ldop-nexus:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-nexus:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
